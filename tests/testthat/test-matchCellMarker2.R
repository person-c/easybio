...
test_that("matchCellMarker2 returns expected structure with default params", {
...
test_that("matchCellMarker2 results are ordered by N descending within each cluster", {
  res <- matchCellMarker2(pbmc.markers, n = 30, spc = "Human")
...
test_that("matchCellMarker2 stores ref, is_custom_ref, and filter_args as attributes", {
  res <- matchCellMarker2(pbmc.markers, n = 30, spc = "Human")
...
test_that("matchCellMarker2 works with custom reference", {
  ...
  res <- matchCellMarker2(pbmc.markers, n = 50, ref = custom_ref)
  expect_s3_class(res, "data.table")
  expect_true(attr(res, "is_custom_ref"))
  expect_equal(nrow(attr(res, "ref")), nrow(custom_ref))
})

test_that("matchCellMarker2 filters by avg_log2FC threshold", {
  res_strict <- matchCellMarker2(pbmc.markers, n = 50, spc = "Human",
                                  avg_log2FC_threshold = 0.5)
  res_loose   <- matchCellMarker2(pbmc.markers, n = 50, spc = "Human",
                                  avg_log2FC_threshold = 0)

  expect_true(nrow(res_strict) > 0)
})

test_that("matchCellMarker2 filters by p_val_adj threshold", {
  res_strict <- matchCellMarker2(pbmc.markers, n = 50, spc = "Human",
...
test_that("matchCellMarker2 returns empty dt when no markers pass filter", {
  res <- matchCellMarker2(pbmc.markers, n = 10, spc = "Human",
                           avg_log2FC_threshold = 100, p_val_adj_threshold = 1e-300)
  expect_equal(nrow(res), 0)
})

test_that("matchCellMarker2 respects tissueClass filter", {
  res_all <- matchCellMarker2(pbmc.markers, n = 30, spc = "Human")
  res_blood <- matchCellMarker2(pbmc.markers, n = 30, spc = "Human",
                                 tissueClass = "Blood")

  expect_true(nrow(res_blood) <= nrow(res_all))
})

test_that("matchCellMarker2 handles n larger than available markers per cluster", {
  # n = 10000 is larger than any cluster has markers
  res <- matchCellMarker2(pbmc.markers, n = 10000, spc = "Human")
  expect_s3_class(res, "data.table")
