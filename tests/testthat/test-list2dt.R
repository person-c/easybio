test_that("list2dt converts a named list to a data.table", {
...
test_that("list2dt accepts custom column names", {
  x <- list(a = "foo", b = c("bar", "baz"))
  res <- list2dt(x, col_names = c("group", "item"))
  expect_equal(colnames(res), c("group", "item"))
})

test_that("list2dt handles single-element list", {
  x <- list(only = c(1, 2, 3))
  res <- list2dt(x)
...
test_that("list2dt handles empty elements in list", {
...
test_that("list2dt handles character vectors", {
